$(function () {
    ...
    const getTokenFrmRefreshToken = async (refresh_token) =>{
        ...
        return await $.ajax({
            ...
            success:function(response){
				...
				//const accessTokenExpiryTime = Date.now() + expiresIn * 60000;
				const accessTokenExpiryTime = new Date(currentTime.setMinutes(currentTime.getMinutes() + expiresIn/60));
    ...
    $(document).on("click", ".saveComponent", function (e) {
		...
		const indexPosition = e.currentTarget.closest("[data-index]").parentElement.closest("[data-index]").getAttribute("data-index");
    ...
    const updateComponent = async (tcmid, that, indexPosition) => {
            ...
            if(inputField.find("input[type=text]").attr("name")!==undefined){
				...
				that.closest("[data-fieldname]")[0].innerHTML=inputValue
            ...
            const checkoutResponse = await postService(checkoutUrl,{});
            
			// const inputValue = inputField.val();
			if(checkoutResponse){
				const schemId = checkoutResponse.Schema.IdRef.split(":").join("_")
			//const schemaResponse = await getSchemaDetails(schemId);
			if(checkoutResponse.Content[tagName]!==undefined && checkoutResponse.Content[tagName]!=="$type"){
				...
				if(checkoutResponse.Content[tagName]===undefined){
					for(item in checkoutResponse.Content){
						console.log(item)
						if(checkoutResponse.Content[item]!==null && checkoutResponse.Content[item][indexPosition]!==undefined){
							if(checkoutResponse.Content[item][indexPosition].hasOwnProperty(tagName)){
								checkoutResponse.Content[item][indexPosition][tagName] = inputValue;
							}
						} else {
							if(checkoutResponse.Content[item]!==null &&  checkoutResponse.Content[item].hasOwnProperty(tagName)){
								checkoutResponse.Content[item][tagName] = inputValue;
							}
						}
					}						
				} 
			...
			//Component Update
			const updateResponse = await putService(updateUrl,checkoutResponse);                   
			if(updateResponse){
				...
				const checkInResponse = await postService(checkinUrl, checkInData);				
				if(checkInResponse){
    ...
    const getFieldType = async(fieldName,value, schemid) => {
		if(value!==""){
			const componentId = value.closest("[data-component-id]").getAttribute("data-component-id");
			const fieldName = value.closest("[data-fieldname]").getAttribute("data-fieldname")
			const data = {
				[componentId]:{
					[fieldName] : value.textContent
				}
			}
			const storageData = JSON.parse(localStorage.getItem("clonedText"));
			if(storageData!==null && storageData[componentId]!==undefined){
				//if(storageData[componentId][fieldName]===undefined){
					storageData[componentId][fieldName] = value.textContent
				//}
				localStorage.setItem("clonedText", JSON.stringify(storageData))
			} else {
				const mergeData = {...data}
				localStorage.setItem("clonedText", JSON.stringify(mergeData))
			}
        } 
		
        const url =  `/items/${schemid}?useDynamicVersion=false`;
        const response = await getRequest(url);
		const xmpliteloading = $(".xpmlite-loading");
		if(xmpliteloading.length!==0){
			$(".xpmlite-loading").remove();
		}
        for (const key in response.Fields) {
            if(key===fieldName){
				...
				if(response.Fields[fieldName].$type==="MultiLineTextFieldDefinition"){
					inputFieldEditor(fieldName, value)
				}
            } 
    ...
    $.fn.cancelEditing = function (e){
		...
		const fieldName = e.currentTarget.closest("[data-fieldname]").getAttribute("data-fieldname")
        const originalText  = JSON.parse(localStorage.getItem("clonedText"))[componentId][fieldName]
	...
	$.fn.showEditor = function (target) {
		...
		if(currentTarget.type!=="text"){
			$("<div class='xpmlite-loading'>Loading...</div>").insertAfter(currentTarget.closest("[data-fieldname]"));
